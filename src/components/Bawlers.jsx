...
const Bawlers = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/products')
            .then(res => {
                const bawlers = res.data.Products.filter(product => product.isApparel);
                setProducts(bawlers);
            })
            .catch(err => {
                console.log(err);
    ...
    return (
        <div className="bg-cover bg-center h-screen flex flex-col items-center overflow-auto" style={bgStyle}>
            {products.length > 0 ? (
                <div className="mt-8">
                    <h2 className="text-5xl font-bold mb-4 text-pink-900 backdrop-blur-sm rounded-lg shadow-md backdrop-hue-rotate-180">Bawlers Apparel</h2>
